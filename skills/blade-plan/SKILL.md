...
name: blade-plan
description: >
  BladeX 轻量规划执行工作流 Blade Spec 简化版。将中等复杂度需求快速拆解为「分析规划 → 逐任务执行 → 完成总结」
  ...
  不适用于简单单文件修改（直接对话即可），也不适用于需要完整架构设计的复杂系统（使用 /blade-spec）。
...
**与 Blade Spec 的关键区别**：
- 不生成任何辅助文件（无 spec.json、tasks.json、requirements.md、design.md）
- 用 Claude Code 内置 TaskCreate / TaskUpdate 追踪进度
- 所有规划直接在对话中呈现，而非写入磁盘
- 中等任务通常一次会话完成，不需要跨会话断点续传
- 三步流程代替五阶段门控，只需一次确认即可开始执行
...
这一步对应 Blade Spec 的需求分析 + 技术设计 + 任务拆解三个阶段的精华浓缩。不写文档，但思考深度不打折扣。
...
先看同模块现有代码怎么写的，再动手。代码风格、命名习惯、继承关系、分层结构——模仿现有代码是最可靠的规范来源。如有 CLAUDE.md，严格遵循其中的编码规范。在 BladeX 体系中，Boot 和 Cloud 共享统一分层：Controller → Service → Mapper → Wrapper → Entity，差异仅在包路径和路由前缀。
...
编写新代码前，先在项目中搜索是否已有可复用的工具类、基类或配置。尤其在 BladeX 体系中，`blade-core-tool` 和各 Starter 模块提供了大量基础设施，重复造轮子不仅浪费时间，还会引入风格不一致。
...
如果在规划阶段发现以下信号，主动建议用户切换到 `/blade-spec`：

- 任务数量超过 15 个，工作量超出一次会话的合理范围
- 涉及复杂的数据库表结构设计（多表关联、索引策略）
- 需要跨多个服务的 API 协调设计
- 需求本身还不清晰，需要详细的需求分析文档来对齐理解
- 影响面广，需要正式的技术设计文档作为团队沟通载体

主动提示：`这个需求的复杂度较高，建议使用 /blade-spec 进行完整的规范驱动开发。`
