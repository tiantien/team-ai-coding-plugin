...
本文档包含 BladeX 后端各层的完整代码模板。所有模板均来源于 BladeX 真实业务代码。
...
### 模式A：TenantEntity（租户隔离，默认）

```java
package org.springblade.modules.{module}.pojo.entity;
...
public class {Name}Entity extends TenantEntity {
...
```java
// 数据审计（记录变更历史）
@DataRecord(module = "{模块名}", operation = "{操作名}", level = DataRecordLevel.INFO)

// 字段级审计
@FieldRecord(description = "{字段描述}")
private Integer category;
...
// Cloud
package org.springblade.{module}.pojo.entity;
```
...
```java
package org.springblade.modules.{module}.pojo.dto;

import lombok.Data;
import lombok.EqualsAndHashCode;
import org.springblade.modules.{module}.pojo.entity.{Name}Entity;
...
@Data
@EqualsAndHashCode(callSuper = true)
public class {Name}DTO extends {Name}Entity {
}
```
...
```java
package org.springblade.modules.{module}.pojo.vo;
...
@Data
@EqualsAndHashCode(callSuper = true)
public class {Name}VO extends {Name}Entity {
...
@Data
@EqualsAndHashCode(callSuper = true)
public class {Name}VO extends {Name}Entity implements INode<{Name}VO> {
...
import org.springblade.modules.{module}.pojo.entity.{Name}Entity;
import org.springblade.modules.{module}.pojo.vo.{Name}VO;
...
public interface {Name}Mapper extends BaseMapper<{Name}Entity> {
     ...
     * 导出数据查询
     */
    List<{Name}Excel> export{Name}(@Param("ew") Wrapper<{Name}Entity> queryWrapper);
}
```
...
<mapper namespace="org.springblade.modules.{module}.mapper.{Name}Mapper">
    ...
    <!-- 通用查询映射结果 -->
    <resultMap id="{modelCode}ResultMap" type="org.springblade.modules.{module}.pojo.entity.{Name}Entity">
 ...
 * @author {author}
 */
public interface I{Name}Service extends BaseService<{Name}Entity> {
 ...
 * @author {author}
 */
public interface I{Name}Service extends IService<{Name}Entity> {
}
```
 ...
 */
@Service
public class {Name}ServiceImpl extends BaseServiceImpl<{Name}Mapper, {Name}Entity> implements I{Name}Service {
 ...
 */
@Service
public class {Name}ServiceImpl extends ServiceImpl<{Name}Mapper, {Name}Entity> implements I{Name}Service {
}
```
...
public class {Name}Controller extends BladeController {
    ...
    public R<{Name}VO> detail({Name}Entity {modelCode}) {
        {Name}Entity detail = {modelCode}Service.getOne(Condition.getQueryWrapper({modelCode}));
    ...
    public R<IPage<{Name}VO>> list(@RequestParam Map<String, Object> {modelCode}, Query query) {
        IPage<{Name}Entity> pages = {modelCode}Service.page(
            Condition.getPage(query),
            Condition.getQueryWrapper({modelCode}, {Name}Entity.class)
        );
    ...
    public R save(@Valid @RequestBody {Name}Entity {modelCode}) {
    ...
    public R update(@Valid @RequestBody {Name}Entity {modelCode}) {
    ...
    public R submit(@Valid @RequestBody {Name}Entity {modelCode}) {
...
import org.springblade.{module}.pojo.entity.{Name}Entity;
import org.springblade.{module}.pojo.vo.{Name}VO;
...
```java
@GetMapping("/detail")
public R<{Name}Entity> detail({Name}Entity {modelCode}) {
    {Name}Entity detail = {modelCode}Service.getOne(Condition.getQueryWrapper({modelCode}));
    return R.data(detail);
}

@GetMapping("/list")
public R<IPage<{Name}Entity>> list(@RequestParam Map<String, Object> {modelCode}, Query query) {
    IPage<{Name}Entity> pages = {modelCode}Service.page(
        Condition.getPage(query),
        Condition.getQueryWrapper({modelCode}, {Name}Entity.class)
    );
...
| 场景 | 类注解 | 方法注解 |
|------|--------|----------|
| 租户业务表 | `@TenantDS` + `@PreAuth(menu = "xxx")` | 无特殊 |
| 系统全局表 | `@NonDS` | `@IsAdmin` |
| 超管专属 | `@NonDS` | `@IsAdministrator` |
| XSS 豁免 | - | `@XssIgnore`（富文本字段） |
...
public void export{Name}(@RequestParam Map<String, Object> {modelCode}, HttpServletResponse response) {
    QueryWrapper<{Name}Entity> queryWrapper = Condition.getQueryWrapper({modelCode}, {Name}Entity.class);
 ...
 * @author {author}
 */
public class {Name}Wrapper extends BaseEntityWrapper<{Name}Entity, {Name}VO> {
    ...
    @Override
    public {Name}VO entityVO({Name}Entity {modelCode}) {
        {Name}VO {modelCode}VO = BeanUtil.copyProperties({modelCode}, {Name}VO.class);
        // 字典翻译：将字典键值转换为可读名称
        String categoryName = DictCache.getValue(DictEnum.{DICT_ENUM}, {modelCode}.getCategory());
...
```java
// 1. 字典翻译
String categoryName = DictCache.getValue(DictEnum.XXX, entity.getCategory());
...
if (Func.equals(entity.getParentId(), BladeConstant.TOP_PARENT_ID)) {
    ...
    Entity parent = SomeCache.getById(entity.getParentId());
...
```java
public List<{Name}VO> listNodeVO(List<{Name}Entity> list) {
...
public interface I{Name}Client {
     ...
     */
    @GetMapping(TOP)
    BladePage<{Name}Entity> top(@RequestParam("current") Integer current, @RequestParam("size") Integer size);
}
```
...
public class {Name}Client implements I{Name}Client {
    ...
    @Override
    @GetMapping(TOP)
    public BladePage<{Name}Entity> top(Integer current, Integer size) {
        Query query = new Query();
        query.setCurrent(current);
        query.setSize(size);
        IPage<{Name}Entity> page = service.page(Condition.getPage(query));
