---
name: blade-design
description: BladeX 全栈代码生成器。根据用户提供的模块名、实体名和字段列表，自动生成符合 BladeX 框架规范的后端代码（Boot 单体/Cloud 微服务）、前端页面（Saber3 Options API/Composition API）和多数据库建表语句（MySQL/PostgreSQL/Oracle/SQL Server/达梦/人大金仓/崖山）。当用户说"帮我生成代码"、"创建一个新模块"、"生成 CRUD"、"新建实体"、"建表语句"、"生成前端页面"、"生成后端接口"、"生成 BladeX 代码"、"写一个 xxx 管理功能"、"帮我设计 xxx 表"、"blade-design"等时触发。即使用户只是描述了业务字段或给出了表结构，也应触发此技能来生成对应的 BladeX 规范代码。
---

# BladeX 全栈代码生成器 (blade-design)
...
- 继承 `TenantEntity`，自动拥有 `tenantId` + 全套审计字段
- Service 继承 `BaseService<Entity>` / `BaseServiceImpl<Mapper, Entity>`
- Controller 使用 `@TenantDS` 注解
- 删除使用 `deleteLogic()`（逻辑删除）
- 适用：Notice、Post、User 等绝大多数业务表

### 模式二：BaseEntity（无租户系统表）
- 继承 `BaseEntity`，拥有审计字段但无 `tenantId`
- Service 继承 `BaseService<Entity>` / `BaseServiceImpl<Mapper, Entity>`
- Controller 使用 `@NonDS` 注解
- 适用：Param 等全局配置表

### 模式三：Raw Serializable（原生模式）
- 实现 `Serializable`，所有字段手动定义
- Service 继承 `IService<Entity>` / `ServiceImpl<Mapper, Entity>`
- 不继承 BladeX 基类，使用 MyBatis-Plus 原生 CRUD
- 删除使用 `removeBatchByIds()`
- 适用：Menu、Role、Dict、RecordData 等跨租户系统表
...
3. **逻辑删除字段** `isDeleted` 必须添加 `@TableLogic` 注解
...
8. **Controller 的 remove 方法**：TenantEntity 用 `deleteLogic()`，Raw 模式用 `removeBatchByIds()`
9. **前端权限标识**格式：`{modelCode}_{action}`，如 `notice_add`、`notice_edit`、`notice_delete`、`notice_view`
10. **Entity 类名后缀**：实体类文件名保持 `{Name}Entity` 后缀，但注意在部分现有代码（如 Notice.java）中没有 Entity 后缀，生成时保持与模板一致即可
