# blade-design - BladeX 全栈代码生成器

基于 BladeX 框架规范的智能代码生成 Skill，覆盖后端、前端、数据库三层，支持 Boot 单体和 Cloud 微服务两种架构模式。
...
|------|------|---------|---------|---------|
| **tenant** | `TenantEntity` | 大多数业务表 | 自动继承 | 有 |
| **base** | `BaseEntity` | 全局配置表 | 自动继承 | 无 |
| **raw** | `Serializable` | 跨租户系统表 | 手动定义 | 无 |

## 支持的数据库方言

| 方言 | 引号 | 大小写 | 注释方式 | 兼容组 |
|------|------|--------|---------|--------|
| MySQL | `` ` `` | 小写 | `COMMENT` 内联 | 独立 |
| PostgreSQL | `"` | 小写 | `COMMENT ON` | PG 系 |
| Oracle | `"` | 大写 | `COMMENT ON` | Oracle 系 |
| SQL Server | `[]` | 小写 | `sp_addextendedproperty` | 独立 |
| 达梦 | `"` | 大写 | `COMMENT ON` | 接近 Oracle |
| 人大金仓 | `"` | 小写 | `COMMENT ON` | PG 系 |
| 崖山 | `"` | 大写 | `COMMENT ON` | Oracle 系 |
...
### 仅生成部分
```
帮我生成 Order 的建表语句，要 MySQL 和 PostgreSQL 两种
...
└── references/
    ...
    └── database.md           ← 数据库建表模板（7 种方言 + 菜单 SQL）
```
...
3. 逻辑删除字段 `isDeleted` 添加 `@TableLogic`
